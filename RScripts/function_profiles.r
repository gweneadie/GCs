...
rhoprofile <- function(pars){
  
  # for a set of parameter values, calculate the model, and output a daraframe of the r and rho(r) profile
  model <- limepy$limepy(g = pars[1], phi0 = pars[2], M = pars[3], rh = pars[4])
  
  data.frame(r=model$r, rho=model$rho)
...
rhocredreg <- function(rhos, r.values, regions=c(0.5,0.75,0.95)){
  
  # credible region bounds
  forquantilefun = sort(c((1 - regions)/2, regions+(1 - regions)/2))
  
  # for every profile estimate, create a spline function for the CMP so we can calculate at any M(r), because limepy doesn't have this flexibility
  funrhos <- lapply(X = rhos, FUN = splineprofile, columns=c("r", "rho"))
  
  # get estimates at custom r values
  customrhos <- lapply(funrhos, FUN = function(x) x(r.values))
  
  # put all custom CMPs into one matrix
  temp <- matrix(unlist(customrhos), ncol = length(r.values), byrow=TRUE)
