...
yrangevel = list(c(0,100),c(0,300),c(0,50))
...
# open file to write to
pdf(paste0("../Figures/velocityprofiles_randomsampling_", Sys.Date(), ".pdf"), width = 8, height = 3)
...
for(i in 1:length(resultsfolders)){
  ...
  # make the plot!
  plot(results[[1]]$r, results[[1]]$v2, xlab=Xlab, ylab=Ylabvel[i], xlim = xrange[[i]], ylim=yrangevel[[i]],  type="n", main="", las=1)
...
########### 3. Plot the high Phi0 and low Phi0 results ######################
