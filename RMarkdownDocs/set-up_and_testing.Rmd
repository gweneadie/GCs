...
Astronomers also have a lot of prior information about GCs in general, so we can put informative priors on things like the mass and half-light radius.
...
_Is this representative of our prior information on GC masses? It seems like a GC of 1 million solar masses is really large?_
...
# now try using this model with the mock data to calculate the DF at the data points
badLIMEPYdf$df( mydata$r, mydata$v )
...
```{r}
LIMEPYdf$df( mydata$r, mydata$v )/testpars[3]
```

and a log-likelihood value of 
```{r}
log( LIMEPYdf$df( mydata$r, mydata$v )/testpars[3] )

```

Is this what our function logLike.limepy() returns?
```{r}
logLike.limepy(pars = testpars, dat = mydata)
...
```{r}
logLike.limepy(pars = badpars, dat = mydata)

lessobviousbadpars = c(1.5, 1.5, 1e5, 0.1)

logLike.limepy(pars = lessobviousbadpars, dat=mydata)
```
...
This surface looks slightly problematic from a sampling perspetive. Where is the max log-target value? 
```{r}
conditional[which.max(conditional$target), ]
```
